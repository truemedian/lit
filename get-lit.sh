...
download "${_luvi_url}" "${_luvi_bin}" || exit 1
download "${_lit_url}" "${_lit_zip}" || exit 1
download "${_luvit_url}" "${_luvit_zip}" || exit 1
...
# Cleanup lit.zip and luvit.zip

rm -f "${_lit_zip}" "${_luvit_zip}"
